...
Over the past decade, researchers around the world have generated large amounts of information related to the human gut microbiome. Large scale population studies have cataloged the vast diversity in the human gut microbiome. I wanted to re-visit some of the basic features of the human gut microbiome that are often taken for granted. Such as how many 'species' are present in an individual's gut/fecal sample? In literature, a variety of numbers are menthioned ranging from 100s to 1000s. Similarly, other questions which I am to visualize in this document. `curatedMetagenomicData` is a useful resource for getting basic insights from wealth of publicly available human metagenomes specifically from the gut.
...
#table(rowData(tse)$species, useNA="always")

summary(colData(tse)$observed.species)

hist(colData(tse)$age)
colnames(colData(tse))
table(colData(tse)$age_category)
```
